        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="hidden md:flex flex-1 max-w-2xl items-center gap-2 border border-gray-200 rounded-full px-4 py-1.5 hover:shadow-md transition-shadow">
            ...
            <button
              ...
              className="bg-green-700 hover:bg-green-800 text-white rounded-full p-2 transition-colors"
            >
              <Search size={16} />
            </button>
          </div>
              ...
              {currrentuser && (
                <>
                  ...
                  <div
                    ...
                    className="h-9 w-9 rounded-full bg-green-50 flex items-center justify-center text-green-700 border border-green-100 cursor-pointer"
                  >
                    <User size={20} />
                  </div>
                </>
              ...
              {!currrentuser && (
                <>
                  <button
                    ...
                    className=" border-[1px] hover:bg-green-500 hover:border-green-500 duration-150 shadow-md shadow-green-200 border-green-700 px-6 p-2  text-sm rounded-xl font-semibold"
                  >
                    Login
                  </button>
                </>
      ...
      <div className="md:hidden px-4 pb-4">
        <div className="flex items-center gap-2 border border-gray-200 rounded-full px-4 py-2 bg-gray-50">
          ...
          <input
            ...
            onChange={(e) => setQ(e.target.value)} // ← wire it up
          />
          <button
            onClick={handleSearch}
            className="bg-green-700 text-white rounded-full p-1.5"
          >
            <Search size={14} />
          </button>
        </div>
      ...
      {isMenuOpen && (
        <div className="lg:hidden absolute top-[112px] md:top-16 left-0 w-full bg-white border-t border-gray-100 shadow-xl animate-in slide-in-from-top duration-200">
          <div className="flex flex-col p-4 gap-4">
            ...
            >
              <div
                className={`h-10 w-10 rounded-full ${!currrentuser && "hidden"} bg-gray-100 flex items-center justify-center`}
              >
...
const Action = ({ icon, label, onClick }) => (
  <button
    onClick={onClick}
    className="flex flex-col items-center gap-0.5 cursor-pointer hover:text-green-700 transition-colors"
  >
    {icon}
...
const MobileNavItem = ({ icon, label, onClick }) => (
  <button
    onClick={onClick}
    className="flex items-center gap-4 p-3 hover:bg-gray-50 rounded-xl transition-colors text-gray-700 font-medium"
  >
